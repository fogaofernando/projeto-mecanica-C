#define POS_Y_TOPO 2	//Posição minima das mensagens (com execao do menu principal)

//TAMANHO DA TELA
#define TELA_X 80	
#define TELA_Y 24


//BOTOES DE MENU/CONTROLE
#define ACIMA 72
#define ABAIXO 80
#define ESC 27
#define ENTER 13
#define BACKSPAECE 8


//SIMBOLOS TABELA ASCII 
#define IMG_OP 62					// 16 ou 62
#define BARRA_HORIZONTAL 205		
#define BARRA_LATERAL 186
#define BARRA_DIG01 201
#define BARRA_DIG02 187
#define BARRA_DIG03 200
#define BARRA_DIG04 188
	

//POSICAO DOS OBJETOS DO MENU DE OPCOES
#define POS_X_OPCOES (TELA_X/8)			
#define POS_Y_OPCOES (5)


#define MIN_PESQUISA 1
//PRIMEIRA STRUCT
#define TAM_NOME 40
#define MIN_NOME 1
#define TAM_CPF 11
#define TAM_DESCRICAO_END 50	
#define MIN_DESCRICAO_END 3
#define MIN_CIDADE 2
#define TAM_CIDADE 20
#define MIN_ESTADO 1
#define TAM_ESTADO 20
#define MIN_TELEFONE 10
#define TAM_TELEFONE 11



//SEGUNDA STRUCT
#define TAM_PLACA 7
#define MIN_MODELO 1
#define TAM_MODELO 20
#define TAM_FABRICANTE 20
#define MIN_FABRICANTE 2
#define TAM_CHASSI 17
#define TAM_ANO 2016
#define MIN_ANO 1960

//TERCEIRA STRUCT
#define TAM_DESCRICAO_MANUTENCAO 500
#define MIN_DESCRICAO_MANUTENCAO 2
#define TAM_VALOR 100000
#define MIN_VALOR 1
#define TAM_MAODEOBRA 100000
#define MIN_MAODEOBRA 10
#define MAX_DATA 12



//PRINCIPAL.C
#define NOME_OPCOES 100				//definir o tamanho das strings do menu de opcao
#define TAM_PROPRIETARIOS 500
#define TAM_VEICULOS 500
#define TAM_MANUTENCAO 500


//Caracteres da Tabela ASCII
#define TAB_A 65
#define TAB_Z 90
#define TAB_a 97
#define TAB_z 122
#define TAB_0 48
#define TAB_9 57
#define TAB_INTERROGACAO 32 //minimo	
#define TAB_VIRGULA 44	
#define TAB_BARRA 47	//TAMimo
#define TAB_PONTO 46
#define TAB_ESPACO 32


//Tipo de dado para leitura
#define TIPO_LETRAS 1
#define TIPO_LETRAS_ESPECIAIS 2
#define TIPO_LETRAS_NUMEROS 3
#define TIPO_LETRAS_ESPECIAIS_NUMEROS 4
#define TIPO_INTEIRO 5
#define TIPO_REAL 6


//deseja aceitar espaco ?
#define SIM 1
#define NAO 0

